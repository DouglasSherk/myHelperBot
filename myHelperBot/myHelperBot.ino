...
void motorLCallback(const std_msgs::Int32& speed) {
  motorL.setSpeed(speed.data);
}

void motorRCallback(const std_msgs::Int32& speed) {
  motorR.setSpeed(-speed.data);
}
  ...
  for (uint8_t i = 0; i < SONAR_NUM; i++) { // Loop through all the sensors.
    if (millis() >= pingTimer[i]) {         // Is it this sensor's time to ping?
      ...
      if (i == 0 && currentSensor == SONAR_NUM - 1) {
        processData_Ultrasonic();                         // Sensor ping cycle complete, do something with the results.
      }
...
void processData_Ultrasonic() { // Sensor ping cycle complete, do something with the results.
  static char buf[512];
  buf[0] = 0;
